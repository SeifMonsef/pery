...
class VariationsDropdown extends Component {
  ...
  getOptionValues(optionName) {
    const values = _.uniq(this.props.variations.map((variation) => {
  ...
  // function that triggers when an option value is selected and
  // changes the available values for the rest of the options
  ...
  render() {
    ...
    const dropdownValues = valuesArray.map(valueArray =>
      valueArray.map(value => ({ value, text: value })));


    const dropdowns = options.map((name, index) => (
      <Card.Group itemsPerRow={2} className="custom-group" key={name}>
        <Card.Content >
          <Dropdown className="custom-drop" placeholder={name} fluid selection options={dropdownValues[index]} onChange={(event, data) => this.remakeValues(data)} />{' '}
        </Card.Content>
      </Card.Group>
    ));
